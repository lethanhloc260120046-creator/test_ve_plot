...
for lookback_days in lookback_list:
    ...
    prompt = f"""
    Bạn là một chuyên gia phân tích định lượng (Quantitative Analyst) xuất sắc tại phố Wall.
    Nhiệm vụ của bạn là dự báo chuỗi thời gian giá cổ phiếu (Regular Time Series) dựa trên dữ liệu lịch sử.

    Dưới đây là chuỗi giá đóng cửa của {lookback_days} ngày giao dịch gần nhất (được sắp xếp theo thứ tự thời gian từ cũ đến mới):
    [{input_string}]

    Hãy suy luận trong đầu (KHÔNG in ra quá trình suy luận) theo các bước sau:
    1. Đánh giá Xu hướng chính (Trend) và Gia tốc (Momentum) của chuỗi {lookback_days} ngày này.
    2. Nhận diện và bỏ qua các biến động nhiễu ngẫu nhiên (Local Noise).
    3. Ngoại suy quỹ đạo giá thực tế cho {predict_days} ngày tiếp theo, đảm bảo tính liên tục và không tạo ra các bước nhảy giá (price jumps) phi logic.

    YÊU CẦU ĐẦU RA (NGHIÊM NGẶT):
    Bạn CHỈ ĐƯỢC PHÉP in ra đúng {predict_days} con số dự báo, phân tách nhau bằng dấu phẩy (,).
    Tuyệt đối KHÔNG chào hỏi, KHÔNG giải thích, KHÔNG có thêm bất kỳ chữ cái hay ký tự nào khác.
    Ví dụ định dạng chuẩn: 150.25, 151.10, 150.85...
    """
